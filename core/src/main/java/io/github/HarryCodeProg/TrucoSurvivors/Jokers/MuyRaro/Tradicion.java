...
public class Tradicion extends Joker {

    public Tradicion() {
        super(121, "Tradicion", "Tradicion", "Reactiva todos los jokers con categoría 'TRADICIONAL'",
            Rareza.muyRaro, 8, Joker.FaseActivacion.INDEPENDIENTE,
            CategoriaJoker.TRADICIONAL);
    }

    @Override
    public Joker copiar() {
        Tradicion copia = new Tradicion();
        copiarEstado(copia);
        return copia;
    }
    ...
    public void aplicarEfecto(EventoJuego evento, ContextoJuego ctx, Juego juego) {
        ...
        for (Joker j : jokers) {
            if (j == this) continue;
            if (j.tieneCategoria(CategoriaJoker.TRADICIONAL)) {
